...
def obtener_distancia():
    ...
    distancia = pulse_duration * 17150  # Calcula la distancia en cm
...
def procesar_video(captura):
    while parametros["ejecutar"]:
        ...
        # Verifica si la distancia es corta y manda 2 (detener)
        distancia = obtener_distancia()
        if distancia < 20:  # Ajusta este valor según tus necesidades
            if ultimo_estado_uart != 2:
                ser.write(str(2).encode())  # Detener motores
                print("Distancia corta detectada -> enviar 2")
                ultimo_estado_uart = 2
